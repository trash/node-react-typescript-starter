...
export class Api {
    private static appendAuthHeaders(headers: Headers = new Headers()): Headers {
    ...
    private static handleResponse(response: Response, json = true): Promise<any> {
        if (response.ok) {
            return json
                ? response.json()
    ...
    static post<D = any, T = any>(url: string, data: D): Promise<T> {
        return fetch(Api.getUrl(url), {
            ...
            headers: Api.appendAuthHeaders(new Headers({
