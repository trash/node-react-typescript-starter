...
import {NoteServer, Note, NotePostServer, NotePatchServer} from '../models/note';
...
class NoteService {
    ...
    private async getAllNotes(
        ...
        const notesServer = await Api.get<NoteServer[]>(`/api/v1/notes`, queryParams);
        if (!notesServer) {
            return [];
        }
    ...
    async createNote(
        ...
        const newNote = await Api.post<NotePostServer, NoteServer>('/api/v1/notes', {
            userId: userId,
            note: note,
            date: moment().utc().format(mysqlDateFormat),
            period: datePeriod
